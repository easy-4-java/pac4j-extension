...
public class UsernamePasswordCaptchaAuthenticator implements Authenticator {
    ...
    @Override
    public java.util.Optional<Credentials> validate(CallContext callContext, Credentials credentials){
        ...
        if(isOverRetryRemind(callContext)) {
        ...
        if(isCaptchaRequired() || isOverRetryTimes(callContext)) {
        ...
        boolean validation = captchaResolver.validCaptcha(context, callContext.sessionStore(), upcCredentials.getCaptcha());
            if (!validation) {
        ...
        return java.util.Optional.of(credentials);
    }
    ...
    protected boolean isOverRetryRemind(CallContext callContext) {
        if (null != getFailureCounter() && getFailureCounter().get(callContext.webContext(), callContext.sessionStore(), getRetryTimesKeyAttribute()) == getRetryTimesWhenAccessDenied()) {
            return true;
        }
        return false;
    }
    ...
    protected boolean isOverRetryTimes(CallContext callContext) {
        if (null != getFailureCounter() && getFailureCounter().get(callContext.webContext(), callContext.sessionStore(), getRetryTimesKeyAttribute()) >= getRetryTimesWhenAccessDenied()) {
            return true;
        }
        return false;
